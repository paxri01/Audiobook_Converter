...
#TEMP_CONCAT_MP3="concat_44k.mp3"
TARGET_BITRATE=48
...
{
  local required_commands=("ffmpeg" "ffprobe" "lame")
...
reEncode()
{
  ...
  logMessage "INFO" "Encoding $(basename "$inFile") at ${target_bitrate}k bitrate"
  ...
  while [[ $checkFile -lt 1 ]]; do
    ...
    # Handle small files that may have concatenation issues
    if [[ $outSize -lt $MIN_FILE_SIZE ]]; then
      tempOut="${outFile%.*}_temp.mp3"
      logMessage "WARN" "File size ${outSize} bytes below threshold - applying fix"
      
      if fixSmallFileSize "$inFile" "$outFile" "$tempOut"; then
        checkFile=1
      else
  ...
  logMessage "INFO" "Successfully encoded: $(basename "$outFile") (${outSize} bytes)"
  return 0
}
...
encodeWithCuda()
{
  ...
  logMessage "INFO" "Using CUDA-accelerated encoding"
  ...
  if ffmpeg -hide_banner -loglevel error -stats \
    ...
    logMessage "WARN" "CUDA encoding failed"
    return 1
  fi
}
...
encodeWithCpu()
{
  ...
  logMessage "INFO" "Using CPU-based encoding"
  
  # Use lame for MP3 encoding with quality settings
  if lame --nohist -m m -V 6 -b "$bitrate" "$inFile" "$outFile" >/dev/null 2>&1; then
...
concatenateAudioFiles()
{
  ...
  for file in "${input_files[@]}"; do
    if [[ -f "$file" ]]; then
      echo "file '$(realpath "$file")'" >> "$concat_list"
    else
  ...
  local result=0
  if ffmpeg -hide_banner -loglevel error -f concat -safe 0 \
    -i "$concat_list" \
