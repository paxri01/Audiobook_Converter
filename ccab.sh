...
targetBitrate=48
# Set the temp directory for saved data files.
tmpDir='/tmp'
...
workDir=$(sudo mktemp -d "$tmpDir/ccab/tmp.XXXXX")
...
validateCommand()
{
  ...
  if ! command -v "$cmd" >/dev/null 2>&1; then
    echo "${C5}[RC:99] ERROR, ${C3}$cmd${C0} not available"
    return 1
  fi
}
...
probeFile()
{
  ...
  if [[ -z $_bookTitle ]]; then
    ...
    read -r _tempTitle
    _tempTitle=$(validateInput "$_tempTitle")
  ...
  if [[ -z $_bookAuthor ]]; then
    ...
    read -r _tempAuthor
    _tempAuthor=$(validateInput "$_tempAuthor")
...
searchIt()
{
  ...
  if [[ -f "$HOME/.config/keys" ]]; then
    ...
    echo "${C5}[RC:13] ERROR, ${C0}Config file not found: $HOME/.config/keys"
    exit 13
  fi
...
parseInfo()
{
  ...
  inFile="$tmpDir/book.html"
  tmpInfo="$tmpDir/book.info"
  richInfo="$workDir/rich_info.txt"
  index=$2

  if [[ -f "$inFile" ]]; then
    grep 'rich_product_information' "$tmpDir/book.html" | hxnormalize | sed -r 's/^\s+>/<span>/g' |\
  ...
  read -r tmpTitle
  tmpTitle=$(validateInput "$tmpTitle")
  ...
  read -r tmpSeries
  tmpSeries=$(validateInput "$tmpSeries")
  _seriesName=${tmpSeries:-$_seriesName}
  bookSeries[$index]=$_seriesName
  ...
  if [[ -f /var/log/ccab/bad_narrators.txt ]] && grep -q "${_narrator//,*/}" /var/log/ccab/bad_narrators.txt; then
    echo -e "${C4}WARNING: ${C0}$_narrator is a known bad narrator."
    echo -e "${C6}Press Enter to continue.${C0}"
    read -r ANS
  fi
  ...
  if [[ -z ${bookTitle[$index]} ]]; then
    ...
    read -r _bookTitle
    _bookTitle=$(validateInput "$_bookTitle")
    bookTitle[$index]=$_bookTitle
  fi
  ...
  if [[ -z $_series ]]; then
    ...
    read -r fullSeries
    fullSeries=$(validateInput "$fullSeries")
  else
    fullSeries="$_series"
  fi
  echo -e "${C2}++fullSeries=$fullSeries${C0}"

  bookSeries[$index]="$_series"
...
EOF
  ...
  ${C0}    Released: ${C8}${bookYear[$index]}${C0}
...
classifyIt()
{
  ...
  echo -e "${C8}>>> Checking book category...${C0}"
  while [[ -z $category ]]; do
...
EOM
    ...
    echo -e "${C4}Select book category number: ${C0}\c"
    read -rn 1 category
    if (( ! category >= 1 && ! category < 7 )); then
      unset category
    fi
